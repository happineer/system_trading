...
class TopTrader(QMainWindow, ui):
    ...
    def test(self):
        lg_code = '066570'
        ret = self.kw.get_login_info('ACCOUNT_CNT')
        pdb.set_trace()
        ret = self.kw.get_login_info('ACCNO')
        pdb.set_trace()
        ret = self.kw.get_login_info('USER_ID')
        pdb.set_trace()
        ret = self.kw.get_login_info('USER_NAME')
        pdb.set_trace()
        ret = self.kw.get_login_info('KEY_BSECGB')
        pdb.set_trace()
        ret = self.kw.get_login_info('FIREW_SECGB')
        pdb.set_trace()
